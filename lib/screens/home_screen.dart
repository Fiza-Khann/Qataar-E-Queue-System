...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> loadBranches() async {
    try {
      // Use collectionGroup to fetch all branches in a single query,
      // eliminating the N+1 pattern of querying categories then branches.
      final branchSnapshot =
          await FirebaseFirestore.instance.collectionGroup('branches').get();

      final allBranches = branchSnapshot.docs.map((doc) {
        final data = doc.data();
      ...
      final loadedCategoryIds = <String>{'All'};
      for (var branch in allBranches) {
        loadedCategoryIds.add(branch['categoryId'] as String);
      }

      if (mounted) {
        setState(() {
          branches = allBranches;
          categoryIds = loadedCategoryIds.toList();
        });
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to load services. Please try again.')),
        );
      }
    }
  }
